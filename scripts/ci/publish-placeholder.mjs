/**
 * Publish live2d.ts placeholder packages at 0.0.0 with Trusted Publisher (OIDC) config.
 *
...
if (rest.includes("--publish") || rest.includes("--trust")) {
    fail(
        "flags --publish/--trust removed. Use subcommands: `publish` | `trust` | (default status).",
    );
}
...
const STUBS = [
    ...
    ...NATIVE_STUBS.map((s) => ({
        ...s,
        description: `Placeholder only (${s.name}).`,
    })),
];
...
function loadCache() {
    try {
        ...
        const expectOk =
            expect &&
            expect.repo === TRUST.repo &&
            expect.file === TRUST.file &&
            expect.env === TRUST.env;
        if (!expectOk) {
...
function runNpm(args, opts = {}) {
    ...
    if (token) {
        userConfig = path.join(
            os.tmpdir(),
            `vmz-placeholder-npmrc-${process.pid}`,
        );
        fs.writeFileSync(
            userConfig,
...
function warnLiveViewMiss(name, detail) {
    ...
    console.log(
        "      if npmjs.com already shows the package: wait ~1?min then retry; do not treat as unpublished",
    );
}
...
function trustMatches(cfg) {
    if (cfg?.raw && typeof cfg.raw === "string") {
        return (
            cfg.raw.includes(TRUST.repo) &&
            cfg.raw.includes(TRUST.file) &&
            (cfg.raw.includes(TRUST.env) || cfg.raw.includes("NPM_PUBLISH"))
        );
    }
    ...
    const allowPublish =
        perms.size === 0 ||
        perms.has("createPackage") ||
        perms.has("publish") ||
        perms.has("npm publish");
    const allowStage =
        perms.size === 0 ||
        perms.has("createStagedPackage") ||
        perms.has("stage") ||
        perms.has("npm stage publish");
    return (
        repo === TRUST.repo &&
        file === TRUST.file &&
        (env === TRUST.env || env === "") &&
...
function classifyConfigs(configs) {
    ...
    if (configs.find(trustMatches))
        return { matches: true, matchKind: "loose" };
...
function listTrustLive(name) {
    ...
    try {
        ...
        if (Array.isArray(data?.configurations))
            return { configs: data.configurations };
...
function checkStatus() {
    ...
    console.log(
        `  Trusted Publisher expect: ${TRUST.repo}  ${TRUST.file}  env=${TRUST.env}  [publish + stage]`,
    );
    console.log(`  cache: ${CACHE_PATH}`);
    if (!totpSecretRaw && !otpStatic) {
        console.log(
            "  tip: put NPM_TOTP_SECRET=<base32> in .env.placeholder.local (gitignored), then pnpm placeholder:trust\n",
        );
    } else {
    ...
    for (const spec of STUBS) {
        ...
        if (!ver) {
            ...
            console.log(
                `  ?${spec.name}  package unknown (cannot confirm published)`,
            );
            missingPkg += 1;
            continue;
        }
        if (resolvedVer.liveMiss) {
            warnLiveViewMiss(
                spec.name,
                `using cache ${ver} @ ${cache.packages[spec.name].versionAt ?? "?"}`,
            );
        }
        const verMark = ver === VERSION ? VERSION : `${ver} (!= ${VERSION})`;
        const verSrc =
            resolvedVer.source === "cache"
                ? "cache; live miss"
                : resolvedVer.source;
        ...
        if (resolved.authRequired && !resolved.trust) {
            console.log(
                `  ? ${spec.name}@${verMark}  trust unknown (EOTP; set NPM_TOTP_SECRET) [${verSrc}]`,
            );
            unknownTrust += 1;
            continue;
        }
        if (!resolved.trust) {
            console.log(
                `  ? ${spec.name}@${verMark}  trust unknown (no cache; run trust with NPM_TOTP_SECRET) [${verSrc}]`,
            );
            unknownTrust += 1;
            continue;
        }
        ...
        if (t.matches) {
            console.log(
                `  ?${spec.name}@${verMark}  trust ok [trust=${src}; pkg=${verSrc}]`,
            );
            ok += 1;
        } else if (t.matchKind === "none") {
            console.log(
                `  ~ ${spec.name}@${verMark}  trust missing [trust=${src}; pkg=${verSrc}]`,
            );
            badTrust += 1;
        } else {
            console.log(
                `  ~ ${spec.name}@${verMark}  trust mismatch [trust=${src}; pkg=${verSrc}]`,
            );
            badTrust += 1;
        }
    }
    ...
    console.log(
        "  next: pnpm placeholder:publish   or   put NPM_TOTP_SECRET in .env.placeholder.local && pnpm placeholder:trust",
    );
...
function writeStub(spec, dir, authToken) {
    ...
    fs.writeFileSync(
        ...
        `# ${spec.name}\n\nPlaceholder package (${VERSION}). Reserved for doki-land/live2d.ts.\n`,
    );
    if (authToken) {
        fs.writeFileSync(
            path.join(dir, ".npmrc"),
...
function cmdPublish() {
    ...
    if (!token) {
        console.warn(
            "placeholder publish: no --token; interactive login may ask OTP (2FA). Tip: --token <npm_access_token> or --otp <code>",
        );
    }

    const root = fs.mkdtempSync(path.join(os.tmpdir(), "live2d-npm-stubs-"));
    console.log(`placeholder publish: stubs ?${root}`);
    console.log(
        dryRun ? "placeholder publish: DRY-RUN" : "placeholder publish: LIVE",
    );
    console.log(`  cache: ${CACHE_PATH}`);
    console.log(
        "  rule: cache version===0.0.0 ?skip; --refresh forces live npm view. Partial retry OK.\n",
    );
    ...
    for (const spec of STUBS) {
        ...
        if (!refresh && !dryRun && cachedVer === VERSION) {
            console.log(
                `  ?${spec.name}@${VERSION}  already published ?skip [cache ${cache.packages[spec.name].versionAt ?? ""}]`,
            );
            skipped += 1;
            continue;
        }
        ...
        if (existing === VERSION && !dryRun) {
            console.log(
                `  ?${spec.name}@${VERSION}  already published ?skip [live]`,
            );
            skipped += 1;
            continue;
        }
        ...
        if (!existing && cachedVer === VERSION && !dryRun && !refresh) {
            ...
            console.log(
                `  ?${spec.name}@${VERSION}  already published ?skip [cache; live miss]`,
            );
            skipped += 1;
            continue;
        }
        ...
        if (!existing && !cachedVer) {
            ...
            warnLiveViewMiss(
                spec.name,
                `cache has ${cachedVer}; live unknown ?will attempt publish`,
            );
        }
        ...
        if (/previously published|cannot publish over/i.test(blob)) {
            console.log(
                `  ?${spec.name}@${VERSION}  already on registry ?skip + cache`,
            );
            skipped += 1;
            cache.packages[spec.name].version = VERSION;
            cache.packages[spec.name].versionAt = new Date().toISOString();
        ...
        saveCache(cache);
        fail(
            `publish failed: ${spec.name} (cache saved ?re-run continues from cached 0.0.0 skips)`,
        );
    }
...
function cmdTrust() {
    ...
    console.log(
        "  rule: skip only when trust list (or cache from a prior list) says already matches\n",
    );
    ...
    for (const spec of STUBS) {
        ...
        if (cached?.matches) {
            console.log(
                `  ?${spec.name}  already configured ?skip [cache ${cached.listedAt}]`,
            );
            skipped += 1;
            continue;
        }
        ...
        if (!ver) {
            warnLiveViewMiss(
                spec.name,
                "no live version and no local cache ?skip trust",
            );
            continue;
        }
        if (resolvedVer.liveMiss) {
            warnLiveViewMiss(
                spec.name,
                `proceeding with cache ${ver}; will still try trust list`,
            );
        }
        ...
        if (live.authRequired) {
            saveCache(cache);
            fail(
                `EOTP while listing ${spec.name} after ${listedThisRun} list(s) this run. Cache saved ?get a fresh OTP and continue.`,
            );
        }
        if (live.error) {
            saveCache(cache);
        ...
        if (entry.matchKind === "mismatch") {
            console.log(
                `  ! ${spec.name}  has trusted publisher that does not match expect:`,
            );
            console.log(
                `    ${JSON.stringify(live.configs, null, 2).split("\n").join("\n    ")}`,
            );
            ...
            if (foreign.length) {
                fail(
                    `${spec.name}: refuse to touch foreign trust (${foreign.map((c) => trustFields(c).repo).join(", ")}). Revoke manually.`,
                );
            }
            if (!stale.length) {
                fail(
                    `${spec.name}: mismatch with no revocable same-repo ids. Fix manually, then --refresh.`,
                );
            }
            ...
            for (const cfg of stale) {
                ...
                if (rr.stdout)
                    process.stdout.write(
                        rr.stdout + (rr.stdout.endsWith("\n") ? "" : "\n"),
                    );
                if (rr.stderr)
                    process.stderr.write(
                        rr.stderr + (rr.stderr.endsWith("\n") ? "" : "\n"),
                    );
                if (rr.status !== 0) {
                    saveCache(cache);
        ...
        if (r.status !== 0) {
            saveCache(cache);
            fail(
                `trust create failed: ${spec.name} (cache kept; fix and retry — do not assume E409 means ok)`,
            );
        }
...
switch (command) {
    ...
    default:
        fail(
            `unknown command \`${command}\`. Use: (default status) | publish | trust`,
        );
}
